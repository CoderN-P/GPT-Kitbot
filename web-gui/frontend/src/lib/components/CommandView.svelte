<script lang="ts">
    ...
    import { Car, CircleDashed, Gauge, Rotate3D, MoveRight, MoveLeft} from "lucide-svelte";
    ...
    $: borderColor = active
        ? (command.command_type === CommandEnum.DRIVE ? 'border-blue-200' : 'border-purple-200')
        : (command.command_type === CommandEnum.DRIVE ? 'border-blue-200/50' : 'border-purple-200/50');

    // Determine text and icon colors based on active state
    $: textColor = active ? 'text-gray-800' : 'text-gray-600';
    $: iconBgColor = active
        ? (command.command_type === CommandEnum.DRIVE ? 'bg-blue-100' : 'bg-purple-100')
        : (command.command_type === CommandEnum.DRIVE ? 'bg-blue-100/50' : 'bg-purple-100/50');
    $: iconColor = active
        ? (command.command_type === CommandEnum.DRIVE ? 'text-blue-600' : 'text-purple-600')
        : (command.command_type === CommandEnum.DRIVE ? 'text-blue-500/80' : 'text-purple-500/80');
...
>
    ...
    {#if active && command.duration}
        <div class="absolute left-0 right-0 bottom-0 h-1 bg-gray-100">
            <div 
                class="h-full {command.command_type === CommandEnum.DRIVE ? 'bg-blue-500' : 'bg-purple-500'}"
                style="width: {progress * 100}%;"
                transition:scale={{ start: 0, opacity: 1, duration: 300, origin: 'left' }}
            ></div>
        </div>
    {/if}

    <!-- Command header with icon -->
    <div class="flex items-center gap-2 mb-3" in:fade={{ delay: entranceDelay + 200, duration: 300 }}>
        ...
        <span class={`font-semibold ${textColor}`}>{formatCommandType(command.command_type)}</span>
        
        <!-- Duration badge -->
        {#if command.duration}
            <span class={`ml-auto text-xs font-medium px-2 py-1 rounded-full ${active ? 'bg-gray-100' : 'bg-gray-100/70'} text-gray-700`}>
                {command.duration}s {#if active && progress > 0 && progress < 1}({Math.round(progress * 100)}%){/if}
            </span>
        {/if}
    </div>
    ...
    <div class="mb-2" in:fade={{ delay: entranceDelay + 300, duration: 300 }}>
        <p class={`${active ? 'text-gray-800' : 'text-gray-600'} font-medium`}>
            {getCommandDescription()} {getDurationDescription()}
        </p>
    </div>
    
    <!-- Technical details with subtle animations -->
    <div class="flex flex-col md:flex-row justify-between gap-3 text-sm" in:fade={{ delay: entranceDelay + 400, duration: 300 }}>
        <div class={`flex flex-row gap-2 items-center p-2 rounded-lg ${active ? 'bg-white/50' : 'bg-white/30'} backdrop-blur-sm`}>
            {#if command.command_type === CommandEnum.DRIVE}
                <Gauge class={`h-4 w-4 ${iconColor}`} />
                <span class={`${active ? 'text-gray-700' : 'text-gray-500'}`}>Speed: <span class="font-medium">{(command.command as DriveCommand).speed}</span></span>
            {:else}
                <MoveRight class={`h-4 w-4 ${iconColor}`} />
                <span class={`${active ? 'text-gray-700' : 'text-gray-500'}`}>Forward: <span class="font-medium">{(command.command as RollerCommand).forward}</span></span>
            {/if}
        </div>
        <div class={`flex flex-row gap-2 items-center p-2 rounded-lg ${active ? 'bg-white/50' : 'bg-white/30'} backdrop-blur-sm`}>
            {#if command.command_type === CommandEnum.DRIVE}
                <Rotate3D class={`h-4 w-4 ${iconColor}`} />
                <span class={`${active ? 'text-gray-700' : 'text-gray-500'}`}>Rotation: <span class="font-medium">{(command.command as DriveCommand).rotation}</span></span>
            {:else}
                <MoveLeft class={`h-4 w-4 ${iconColor}`} />
                <span class={`${active ? 'text-gray-700' : 'text-gray-500'}`}>Backward: <span class="font-medium">{(command.command as RollerCommand).backward}</span></span>
            {/if}
        </div>
    </div>
        ...
        <div class="absolute top-4 right-4 flex items-center justify-center">
            <span class="relative flex h-3 w-3">
                <span class="animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 
                    {command.command_type === CommandEnum.DRIVE ? 'bg-blue-400' : 'bg-purple-400'}"></span>
                <span class="relative inline-flex rounded-full h-3 w-3 
                    {command.command_type === CommandEnum.DRIVE ? 'bg-blue-500' : 'bg-purple-500'}"></span>
            </span>
        </div>
    {/if}
    ...
    {#if command.pause_duration > 0}
        <div class="mt-2 flex items-center gap-2 text-xs text-gray-500" in:fade={{ delay: entranceDelay + 500, duration: 300 }}>
